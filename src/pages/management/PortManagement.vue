        ...
        <el-row>
          ...
          <el-col :span="12">
            <div class="grid-content bg-purple-light">
              <el-button
                :type="dialogData.LinkStatus == 'Down' ? 'danger' : 'success' "
                disabled
...
methods: {
  ...
  reload() {
    ...
    this.$http.requestGet(param).then((result) => {
      ...
      for(let i =0 ;i<result.data[0].length;i++){
        ...
        if(result.data[0][i].DuplexMode == "FULL_DUPLEX"){
          result.data[0][i].DuplexMode = "全双工"
        }else{
          result.data[0][i].DuplexMode = "半双工"
        }
      }
  ...
  setPort(){
    ...
    if(this.hasVLAN.includes(Number(this.dialogData.PVID))){
      ...
      if(this.dialogData.LinkType == 'Hybrid'){
        console.log(typeof(this.dialogData.TaggedVlan))
        console.log(typeof(this.dialogData.TaggedVlan))
        //判断是否为string类型，如果不是则进行split
  ...
  //对话框
  openDialog(event){
    this.dialogData = event.__ob__.value
    //判断速率是否自协商
  ...
  //判断链路状态
  LinkStatusChange(){
    console.log(this.dialogData.LinkStatus)
    if(this.dialogData.LinkStatus == "Up"){
      this.dialogData.LinkStatus = "Down"
    }else{
